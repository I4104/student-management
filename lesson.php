<?php include "handler/config.php"; 
    ...
    $get = $conn->query("SELECT * FROM buoihoc WHERE id = '{$_GET["id"]}'");
    if ($get->num_rows > 0) {
	...
	<div class="wrapper">
		<div class="main-header">
			...
			<div class="main-panel">
				<div class="content">
					<div class="container-fluid">
    					<h4 class="page-title">Buổi học: <?php echo $class["name"]; ?></h4>
    					<div class="row">
							<div class="col-lg-12">
								<div class="card">
								    <div class="card-header">
										<h4 class="card-title">Thông tin cơ bản</h4>
										<p class="card-category">Chỉnh sửa thông tin buổi học</p>
									</div>
									<div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <form id="edit_buoi" method="POST">
                                                    <div class="form-group row">
                                                        <label class="col-sm-3 col-form-label">Tên buổi học:</label>
                                                        <div class="col-sm-9">
                                                            <input class="form-control" type="text" name="name" value="<?php echo $class["name"]; ?>" required>
                                                        </div>
                                                    </div>
        									    </div>
        									    <div class="col-md-6">
                                                    <div class="form-group row">
                                                        <label class="col-sm-3 col-form-label">Số tiết học:</label>
                                                        <div class="col-sm-9">
                                                            <input class="form-control" type="number" name="sotiet" value="<?php echo $class["sotiet"]; ?>" required>
                                                        </div>
                                                    </div>
                                                    <div class="form-group row">
                                                        <label class="col-sm-3 col-form-label">ID buổi học:</label>
                                                        <div class="col-sm-9">
    					...
    					<div class="row">
							<div class="col-md-12">
    							<div class="card">
    								<div class="card-header">
    									<div class="card-title">Buổi học: <?php echo $class["name"]; ?></div>
    								...
    								<div class="card-body">
    									<table class="table table-bordered">
											...
											<tbody>
											    <?php
											        $get = $conn->query("SELECT * FROM class_student WHERE giaovien = '{$class["giaovien"]}' AND lophoc = '{$class["lophoc"]}'");
											        if ($get->num_rows > 0) {
											            $i = 0;
                													...
                													<td>
                    													<div class="form-check">
                															<label class="form-check-label">
                																<input class="form-check-input task-select" data-action="diem-danh" data-buoi="'. $_GET["id"] .'" id="'. $row["id"] .'" type="checkbox" '.$checked.'>
                																<span class="form-check-sign"></span>
                															</label>
                														</div>
                													</td>
