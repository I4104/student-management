							...
							<div class="col-md-12">
    							<div class="card">
    								<div class="card-header">
    								    <button class="btn btn-success float-right" data-toggle="modal" data-target="#addnew">Thêm mới</button>
    									<div class="card-title">Danh sách học sinh</div>
    									<div class="card-category">Danh sách toàn học sinh của bạn</div>
    								</div>
    								<div class="card-body table-responsive">
    									<table class="table table-bordered">
											...
											<tbody>
											    <?php 
											        $students = $conn->query("SELECT * FROM students WHERE giaovien = '{$_SESSION["username"]}'");
											            ...
											            while ($row = $students->fetch_array()) {
											                ...
											                echo '
											                    <tr>
                													...
                													<td>'. $row['ten'] .'</td>
                													<td>'. $row['email'] .'</td>
                													<td class="text-center width-60px"><a data-toggle="tooltip" title="Xóa học sinh?" class="text-danger" onclick="del_student('. $row['id'] .')"><i class="fa fa-trash"></i></a></td>
                												</tr>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Thêm học sinh mới</h5>
                ...
                <div class="modal-body">
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            <a class="nav-link active" data-toggle="tab" href="#thucong" role="tab" aria-controls="nav-home" aria-selected="true">Thủ công</a>
                            <a class="nav-link" data-toggle="tab" href="#excel" role="tab" aria-controls="nav-profile" aria-selected="false">Từ EXCEL</a>
                        </div>
                    </nav>
                    <div class="tab-content" id="nav-tabContent">
                        <div class="tab-pane fade show active" id="thucong" role="tabpanel">
                            <form id="new_student">
                                <div class="form-group">
                                    <label>Mã học sinh</label>
                                    <input type="text" class="form-control shadow-none" name="mahocsinh" required>
                                </div>
                                <div class="form-group">
                                    <label>Họ và tên lót</label>
                                    <input type="text" class="form-control shadow-none" name="ho" required>
                                </div>
                                <div class="form-group">
                                    <label>Tên học sinh</label>
                                    <input type="text" class="form-control shadow-none" name="ten" required>
                                </div>
                                <div class="form-group">
                                    <label>Email</label>
                        ...
                        <div class="tab-pane fade" id="excel" role="tabpanel">
                            <p>* Lưu ý:<br>
                                - Khi import từ excel, toàn bộ dữ liệu học sinh cũ sẽ được xóa bỏ<br>
                                - Dữ liệu file excel bắt buộc có dạng: mã số sinh viên, họ & chữ lót, tên, email và bắt đầu từ dòng số 2<br>
                                Ví dụ: A:HS1000, B:Tạ Đăng, C:Khoa, D:i4104.vn@gmail.com
                            </p>
                            <form id="i_excel" method="POST" encrypt="multipart/form-data">
                                <div class="form-group">
                                    <input type="file" name="excel" class="form-control-file" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" required>
                                </div>
                                <button type="submit" class="btn btn-primary float-right">Save changes</button>
                            </form>
                        </div>
